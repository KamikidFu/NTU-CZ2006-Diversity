...
public class VolunteerClientMgr {
    ...
    public void SetDescriptionUSER(String newDescription){
        mRootRef.child("Volunteer").child(USER.getUid()).child("description").setValue(newDescription);
    }
    public void SetNameUSER(String newName) {
        mRootRef.child("Volunteer").child(USER.getUid()).child("name").setValue(newName);
    }
